...
class TestLeaderAgentConfigTimeout:
    ...
    def test_timeout_default_value(self) -> None:
        ...
        config = LeaderAgentConfig(model="google-gla:gemini-flash-lite-latest")
    ...
    def test_timeout_explicit_value(self) -> None:
        ...
        config = LeaderAgentConfig(model="google-gla:gemini-flash-lite-latest", timeout_seconds=120)
        assert config.timeout_seconds == 120

    def test_timeout_none_value(self) -> None:
        """Test timeout_seconds accepts None (optional field)."""
        config = LeaderAgentConfig(model="google-gla:gemini-flash-lite-latest", timeout_seconds=None)
        assert config.timeout_seconds is None

    def test_timeout_minimum_bound(self) -> None:
        """Test timeout_seconds minimum boundary (10 seconds)."""
        # Valid: exactly 10 seconds
        config = LeaderAgentConfig(model="google-gla:gemini-flash-lite-latest", timeout_seconds=10)
        assert config.timeout_seconds == 10

        # Invalid: below minimum
        with pytest.raises(ValidationError, match="greater than or equal to 10"):
            LeaderAgentConfig(model="google-gla:gemini-flash-lite-latest", timeout_seconds=9)
    ...
    def test_timeout_maximum_bound(self) -> None:
        ...
        config = LeaderAgentConfig(model="google-gla:gemini-flash-lite-latest", timeout_seconds=600)
        assert config.timeout_seconds == 600

        # Invalid: above maximum
        with pytest.raises(ValidationError, match="less than or equal to 600"):
            LeaderAgentConfig(model="google-gla:gemini-flash-lite-latest", timeout_seconds=601)
    ...
    def test_timeout_typical_values(self) -> None:
        ...
        for timeout in typical_values:
            config = LeaderAgentConfig(model="google-gla:gemini-flash-lite-latest", timeout_seconds=timeout)
