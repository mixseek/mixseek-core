...
評価スコア降順、同スコアの場合は作成日時早い順でソートされます。`execution_id`によるフィルタリングも高速化されます。

---

### execution_summary テーブル

オーケストレーション実行の集約結果を保存します。
...
DuckDBのMVCC（Multi-Version Concurrency Control）により、複数チームが同時にデータベースに書き込んでもロック競合が発生しません。
...
**パフォーマンス目標**: 10チーム×5ラウンド=50件の並列書き込みが全て成功

---

### エクスポネンシャルバックオフリトライ

データベース書き込み失敗時、1秒、2秒、4秒の間隔で最大3回リトライします。
...
3回リトライ失敗後は即座にエラー終了し、詳細ログを出力します。

---

## トランザクション管理

すべての書き込み操作はトランザクション内で実行されます。
...
これはData Accuracy Mandateに準拠した設計です。暗黙的なデフォルト値やフォールバックは一切提供されません。
