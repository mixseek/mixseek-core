"""Unit tests for ClarityCoherence metric."""
...
class TestClarityCoherenceMetric:
    """Test suite for ClarityCoherence."""

    def test_metric_name(self):
        """Test that metric name is derived from class name."""
        metric = ClarityCoherence()
        assert type(metric).__name__ == "ClarityCoherence"
    ...
    def test_instruction_contains_key_criteria(self):
        """Test that instruction contains clarity/coherence evaluation criteria."""
        metric = ClarityCoherence()
        instruction = metric.get_instruction()

        # Check for key criteria in the instruction
        assert "明瞭性" in instruction or "一貫性" in instruction
        assert "構造" in instruction
        assert "言語" in instruction
        assert "可読性" in instruction
    ...
    async def test_evaluate_returns_metric_score(self, mock_evaluate):
        ...
        mock_llm_result = BaseLLMEvaluation(score=85.0, evaluator_comment="Clear and well-structured response")
        ...
        assert result.metric_name == "ClarityCoherence"
        assert result.score == 85.0
        assert result.evaluator_comment == "Clear and well-structured response"
    ...
    async def test_evaluate_preserves_comment(self, mock_evaluate):
        ...
        long_comment = "This is a very detailed comment about clarity and coherence. " * 10
    ...
    async def test_evaluate_uses_metric_specific_instruction(self, mock_evaluate):
        """Test that evaluate uses the ClarityCoherence specific instruction."""
        mock_llm_result = BaseLLMEvaluation(score=85.0, evaluator_comment="Good")
        mock_evaluate.return_value = mock_llm_result
    ...
    async def test_user_prompt_includes_structured_output_instruction(self, mock_evaluate):
        ...
        """
        mock_llm_result = BaseLLMEvaluation(score=85.0, evaluator_comment="Test comment")
        mock_evaluate.return_value = mock_llm_result
