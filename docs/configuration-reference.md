...
max_retries = 3
timeout_seconds = 60
judge_on_final_round = true  # 最終ラウンドでもLLM Judgementを実行するか（デフォルト: true）
system_instruction = """
あなたは改善判定の専門家です。
...
export MIXSEEK_JUDGMENT__JUDGE_ON_FINAL_ROUND=false  # 最終ラウンドのLLM呼び出しをスキップ
```

**`judge_on_final_round` の動作**:

`judge_on_final_round` は、最終ラウンド（`current_round >= max_rounds` に到達した時点）の LLM Judgement 呼び出しを制御するフラグです。

| 設定値 | 最終ラウンドでの挙動 |
|-------|------------------|
| `true`（デフォルト） | 通常通り LLM Judgement を呼び出し、その結果を `reasoning` として記録する。ただし max_rounds に到達しているため、結果の `should_continue` に関係なくラウンドは停止する |
| `false` | LLM 呼び出しをスキップし、継続判定を `should_continue=False`、`confidence_score=1.0`、`reasoning="Final round reached (judgment skipped by judge_on_final_round=False)"` として DuckDB に記録する |

**使い分け**:

- **最終ラウンドでの無意味な判定を省きたい場合（主目的）**: `false` を設定すると、`should_continue` の結果が実行制御に寄与しない最終ラウンドでのLLM呼び出しをスキップできる
- **判定理由ログを残したい場合**: `true` のまま運用し、最終ラウンドでも LLM による `reasoning` を DuckDB に記録する（監査・分析用途）

> **補足**: 最終ラウンドの LLM 判定は `should_continue` の値に関わらず次のラウンドへ進まないため、判定結果そのものは実行制御に寄与しません。副次的な効果としてAPI費用の削減にもつながります。デフォルトで `true` になっているのは後方互換性のためです。
...
2. **ラウンドmin_rounds+1〜max_rounds-1**: 各ラウンド後にJudgment判定
   - `should_continue=True`: 次のラウンドを実行
   - `should_continue=False`: 実行を停止し、最良のラウンドを選択
3. **max_rounds到達（最終ラウンド）**: 自動停止
   - `judge_on_final_round=true`（デフォルト）: LLM Judgementを実行し、`reasoning` をログに記録した上で停止
   - `judge_on_final_round=false`: LLM Judgementをスキップし、`should_continue=False` として記録した上で停止（最終ラウンドでの無意味な判定を回避する主目的、副次的にAPI費用も削減）
...
プロンプト整形はRoundControllerがUserPromptBuilderで行います。
...
- `$MIXSEEK_WORKSPACE`未設定時は`EnvironmentError`が発生
