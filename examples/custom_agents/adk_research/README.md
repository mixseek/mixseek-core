...
| 目標値                            | 検証方法                     |
| --------------------------------- | ---------------------------- |
| 単一検索処理時間 < 10 秒          | E2E テストの実行時間測定     |
| 深層調査処理時間 < 30 秒          | E2E テストの実行時間測定     |
| エラー情報の 100%取得             | 単体テストによる検証         |
| ソースコード追跡の 100%実現       | 単体テストによる検証         |
| コードカバレッジ 80%              | カバレッジレポートによる測定 |
| MemberAgentResult との 100%互換性 | 統合テストによる検証         |
